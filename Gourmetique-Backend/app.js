...
app.post("/cusreg", async (req, res) => {
  try {
    const { name, mobileno, email, address, gender, username, password } =
      req.body;
...
app.post("/add-starter", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Starter added successfully", data: newStarter });
  } catch (error) {
...
app.post("/add-mains", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Main Course added successfully", data: newMains });
  } catch (error) {
...
app.post("/add-dessert", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Dessert added successfully", data: newDessert });
  } catch (error) {
...
app.post("/viewsprestaurant", async (req, res) => {
  try {
    ...
    const restaurant = await restaurantregModel
      .findById(restaurantId)
      .select("restaurantName address phone");
...
app.get("/viewadmincusads", async (req, res) => {
  try {
    ...
    const adsWithRestaurantNames = await Promise.all(
      viewads.map(async (adDoc) => {
...
app.post("/vieworders", async (req, res) => {
  ...
  try {
    ...
    const acceptedOrderIds = acceptedOrders.map((order) =>
      order.orderId.toString()
    );

    // Step 3: Filter out accepted orders
    const newOrders = orders.filter(
      (order) => !acceptedOrderIds.includes(order._id.toString())
    );
...
app.post("/getAcceptedOrders", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "Error retrieving accepted orders.", error });
  }
});
  ...
  try {
    ...
    res.status(200).send({
      message: `Deleted item: ${
        deletedItem.item || deletedItem.mitem || deletedItem.ditem
      }`,
    });
  } catch (error) {
